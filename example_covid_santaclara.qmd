...
W_grid <- seq(from = 0.0001, to = 0.01, by = 0.0005)
...
opts.mcs$use_discount <- TRUE
opts.mcs$use_custom <- TRUE
opts.mcs$custom_discount_factor <- 0.92
...
if ("forecast" %in% names(out.mcs$error)) {
    if ("y_cast" %in% names(out.mcs$error$forecast)) {
...
if ("mu0" %in% names(out.mcs$fit)) {
    p6 <- ggplot(data.frame(mu0 = c(out.mcs$fit$mu0)), aes(x = mu0)) +
        stat_bin(bins = 50) +
        theme_light()
...
if ("mu0" %in% names(out.mcs2$fit)) {
    p7 <- ggplot(data.frame(mu0 = c(out.mcs2$fit$mu0)), aes(x = mu0)) +
        stat_bin(bins = 50) +
        theme_light()
