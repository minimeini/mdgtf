...
Run `quarto render /Users/meinitang/Dropbox/Repository/poisson-dlm/example_covid_santaclara.qmd --to pdf` to compile.
...
2022-07-08 End of CDC's COVID-19 timeline.
...
y2 <- c(0, data2$cases)
y2_extended <- data[data$date >= "2021-12-01" & data$date <= "2022-01-01", ]$cases
...
#| include: false
opts.lba$use_discount <- "W" %in% param_infer
out.lba <- dgtf_infer(
    mod, y2,
    "lba", opts.lba,
...
opts.mcs$use_discount <- "W" %in% param_infer
out.mcs <- dgtf_infer(
    mod, y2,
    "mcs", opts.mcs,
    forecast_error = eval_forecast_err,
...
```{r}
forecast.hva <- dgtf_forecast(
    mod, y2, out.hva$fit$psi_stored, out.hva$fit$W,
    mu0 = mod$param$obs[1],
    ycast_true = y2_extended[1:10], loss_func = "quadratic", k = 10
)
```
...
To slow doing bi-monthly, quarterly is more appropriate.
...
if (save_data) {
    fname <- paste0(tag, "-mcmc-W-bi-monthly.RData")
    var_list <- c("ddate0", "idx0", "ytrue", "ycast")
    var_list <- var_list[sapply(var_list, exists)]
    save(file = file.path(opath, "data", fname), list = var_list)
}
```
...
if (save_data) {
    fname <- paste0(tag, "-mcmc-W-second-wave.RData")
    var_list <- c("second_wave_date", "second_wave_idx", "second_wave_ytrue", "second_wave_ycast")
    var_list <- var_list[sapply(var_list, exists)]
    save(file = file.path(opath, "data", fname), list = var_list)
}
```
...
if (save_data) {
    fname <- paste0(tag, "-mcmc-W-third-wave.RData")
    var_list <- c("third_wave_date", "third_wave_idx", "third_wave_ytrue", "third_wave_ycast")
    var_list <- var_list[sapply(var_list, exists)]
    save(file = file.path(opath, "data", fname), list = var_list)
}
```
...
if (save_data) {
    fname <- paste0(tag, "-mcmc-W-key-dates.RData")
    var_list <- c("key_date", "key_idx", "key_ytrue", "key_ycast")
    var_list <- var_list[sapply(var_list, exists)]
    save(file = file.path(opath, "data", fname), list = var_list)
}
```