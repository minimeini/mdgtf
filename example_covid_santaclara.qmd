...
save_figure <- function(algo_name, tag, p_psi_post, p_y_filter, p_y_smooth, plist_forecast, opath) {
    label <- paste0("covid-", tag, "-", algo_name, "-")

    ggsave(
        file.path(
            opath, "posterior",
            paste0(label, "posterior-psi.pdf")
        ),
    ...
    for (j in 1:kstep_forecast_err) {
        ggsave(
            file.path(
                opath, "forecast",
                paste0(label, paste0("forecast-", j, "step.pdf"))
            ),
...
opts.lba$custom_discount_factor <- 0.9
```
...
out.lba2.loss_all <- c(
    c(out.lba2$error$forecast$y_loss_all),
    out.lba2$error$fitted$filter$y_loss_all,
    out.lba2$error$fitted$smooth$y_loss_all
)

names(out.lba2.loss_all) <- c(
    paste0("forecast", 1:length(c(out.lba2$error$forecast$y_loss_all))),
    "fitted-filter", "fitted-smooth"
)
print(out.lba2.loss_all)
```
...
opts.mcs$use_discount <- TRUE
opts.mcs$use_custom <- TRUE
opts.mcs$custom_discount_factor <- 0.85
...
out.mcs2.loss_all <- c(
    c(out.mcs2$error$forecast$y_loss_all),
    c(out.mcs2$error$fitted$filter$y_loss_all),
    c(out.mcs2$error$fitted$smooth$y_loss_all)
)
names(out.mcs2.loss_all) <- c(
    paste0("forecast", 1:length(c(out.mcs2$error$forecast$y_loss_all))),
    "fitted-filter", "fitted-smooth"
)
print(out.mcs2.loss_all)
```
...
p2 <- plot_ts_ci_single(out.mcs$fit$psi, main = "Posterior distribution of psi")
plot(p2)
...
opts.ffbs$use_discount <- TRUE
opts.ffbs$use_custom <- TRUE
opts.ffbs$custom_discount_factor <- 0.91
...
out.ffbs2.loss_all <- c(
    c(out.ffbs2$error$forecast$y_loss_all),
    out.ffbs2$error$fitted$filter$y_loss_all,
    out.ffbs2$error$fitted$smooth$y_loss_all
)

names(out.ffbs2.loss_all) <- c(
    paste0("forecast", 1:length(out.ffbs2$error$forecast$y_loss_all)),
    "fitted-filter", "fitted-smooth"
)

print(out.ffbs2.loss_all)
...
p2 <- plot_ts_ci_single(out.pl$fit$psi, main = "Posterior distribution of psi")
plot(p2)
...
if (mode == "save") {
    ...
    ggsave(
        file.path(
            opath, "W",
            paste0("covid-", tag, "-posterior-W.pdf")
        ),
...
if (mode == "save") {
    ggsave(
        file.path(
            opath, "hva",
            paste0("covid-", tag, "-hva-optimal-learning-rate.pdf")
        ),
        plot = p1, device = "pdf", dpi = 300
    )
    ...
    ggsave(
        file.path(
            opath, "hva",
            paste0("covid-", tag, "-hva-step-size.pdf")
        ),
        plot = p2, device = "pdf", dpi = 300
    )
}
...
p2 <- plot_ts_ci_single(out.hva$fit$psi, main = "Posterior distribution of psi")
plot(p2)
...
p2 <- plot_ts_ci_single(out.mcmc$fit$psi, main = "Posterior distribution of psi")
plot(p2)
...
if (mode == "save") {
    ggsave(
        file.path(
            ...
            paste0("covid-", tag, "-lag-lognorm.pdf")
        ),
    ...
    ggsave(
        file.path(
            opath, "model-selection",
            paste0("covid-", tag, "-lag-lognorm-heatmap.pdf")
        ),
...
if (mode == "save") {
    ggsave(
        file.path(
            opath, "model-selection",
            paste0("covid-", tag, "-lag-nbinom.pdf")
        ),
        plot = p, device = "pdf", dpi = 300
    )
    ...
    ggsave(
        file.path(
            opath, "model-selection",
            paste0("covid-", tag, "-lag-nbinom-heatmap.pdf")
        ),
        plot = p0, device = "pdf", dpi = 300
    )
}
