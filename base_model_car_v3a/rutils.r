get_posterior_summary = function(output, mod, param_infer) {
    ...
    for (par in param_infer) {
        if (par == "alpha") {
            ...
            par_true = c(par_true, c(mod$param$spatial_effect_alpha))
        } else if (par == "car") {
            par_qt = rbind(
                par_qt,
                quantile(c(output$car_alpha_mu), qt),
                quantile(c(output$car_alpha_tau2), qt),
                quantile(c(output$car_alpha_rho), qt),
            ...
            par_name = c(
                par_name,
                c(
                    "car_alpha_mu", "car_alpha_tau2", "car_alpha_rho", 
                    "car_beta_mu", "car_beta_tau2", "car_beta_rho",
                    "car_wt_tau2", "car_wt_rho"
                )
            )
            par_true = c(par_true, mod$spatial$car_alpha, mod$spatial$car_beta, mod$spatial$car_wt[2:3])
        } else if (par == "par1") {
            par_qt = rbind(
                par_qt,
...
get_mh_diagnostics = function(output) {
    mh_diagnostics = data.frame(
        param = c("global", "local", "log_alpha", "log_beta"),
        accept_rate = c(
    ...
    for (par in param_infer) {
        if (par == "alpha") {
            ...
            par_rhat = c(
                par_rhat,
                gelman_rubin_cpp(
                    t(do.call(rbind, lapply(output, function(x) c(x$car_alpha_mu))))
                ),
                gelman_rubin_cpp(
                    t(do.call(rbind, lapply(output, function(x) c(x$car_alpha_tau2))))
                ),
                gelman_rubin_cpp(
                    t(do.call(rbind, lapply(output, function(x) c(x$car_alpha_rho))))
                ),
                gelman_rubin_cpp(
