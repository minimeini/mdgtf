...
public:
    ...
    Model(const Rcpp::List &settings)
    {
        ...
        if (spatial_settings.containsElementNamed("neighborhood_matrix"))
        {
            ...
            arma::vec npop(nS, arma::fill::ones);
            if (spatial_settings.containsElementNamed("population"))
            {
                npop = Rcpp::as<arma::vec>(spatial_settings["population"]);
                if (npop.n_elem != nS)
                {
                    throw std::invalid_argument("Model::Model - dimension of population vector is not consistent with 'nlocation'.");
                }
            }

            spatial_alpha = SpatialStructure(V, npop);
            spatial_beta = SpatialStructure(V, npop);
            spatial_wt = SpatialStructure(V, npop);
        }
        else
        {
    ...
    )
    {
        ...
        )
        {
            ...
            {
                SpatialStructure spatial_i(spatial_alpha.V, spatial_alpha.npop);
        ...
        )
        {
            ...
            for (unsigned int i = 0; i < nsample; i++)
            {
                SpatialStructure spatial_i(spatial_beta.V, spatial_beta.npop);
                spatial_i.update_params(car_mu.at(i), car_tau2.at(i), car_rho.at(i));
        ...
        for (unsigned int i = 0; i < nsample; i++)
        {
            ...
            for (unsigned int t = 1; t <= ntime; t++)
            {
                for (unsigned int s = 0; s < nS; s++)
                {
                    ...
                    eta += model_i.spatial_beta.npop.at(s) * beta * arma::dot(
                        model_i.spatial_beta.W.row(s).t(), 
                        Y.col(t-1) / model_i.spatial_beta.npop
                    );
    ...
    ) const
    {
        ...
        const double npop = spatial_beta.npop.at(s);
        ...
        for (unsigned int t = 1; t < y.n_elem; t++)
        {
            ...
            // Cross-regional effect
            eta += spatial_effect_beta * npop * arma::dot(neighbor_weights, Y.col(t - 1) / spatial_beta.npop);
    ...
    )
    {
        ...
        for (unsigned int s = 0; s < Y.n_rows; s++)
        {
            ...
            const double spatial_effect_alpha = std::exp(std::min(log_alpha.at(s), UPBND));
            const double spatial_effect_beta = std::exp(std::min(log_beta.at(s), UPBND));
            const arma::vec neighbor_weights = spatial_beta.W.row(s).t() / spatial_beta.npop;
            const arma::vec cross_region_effects = spatial_effect_beta * Y.head_cols(nT).t() * neighbor_weights * spatial_beta.npop.at(s); // nT x 1
    ...
    )
    {
        ...
        for (unsigned int s = 0; s < nS; s++)
        {
            ...
            for (unsigned int t = 1; t < Y.n_cols; t++)
            {
                double deta_dbeta = spatial_beta.npop.at(s) * arma::dot(spatial_beta.W.row(s).t(), Y.col(t - 1) / spatial_beta.npop);
