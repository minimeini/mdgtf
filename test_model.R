...
default_opts = function(n=NA,
  ...
  model_code = .info$get_model_code(obs_dist=obs_dist,
                                    link_func=link_func,
...
test_model = function(n,
  ...
  eta_init = c(sim_dat$params$W,
               ...
               sim_dat$params$psi0)
  MH_var = c(0.05,0.001)
  mcmc_out = mcmc_disturbance_pois(sim_dat$y,
                                   sim_dat$params$model_code,
                                   eta_select,
                                   eta_init,
                                   eta_prior_type,
  ...
  p1 = plot_psi(sim_dat=sim_dat,
                psi_list=list(LBE=lbe_out$psi,MCS=mcs_out$psi,HVB=hvb_out$psi,MCMC=mcmc_out$psi),
...
test_model_real = function(y,opts,
                           eta_select=c(1,0,0,0,0,0),
                           eta_prior_type=c(0,0,0,0,0,0),
                           model_compare=c(1,1,1,1,1,1,1,1),
                           Blag = 30,
                           Blag_pct = 0.15,
                           mcsN=5000,
                           nburnin = 50000,
  ...
  if (model_compare[1]==1) {
    lbe_out1 = lbe_poisson(y,opts$model_code,
                           ...
                           summarize_return=TRUE)
    psi_list = c(psi_list,list(lbe_out1$psi))
    psi_name = c(psi_name,"LBE")
    eta_list = c(eta_list,list(lbe_out1$Wt))
    eta_name = c(eta_name,"LBE")
  }
  
  if (model_compare[2]==1) {
    mcs_out1 = mcs_poisson(y, opts$model_code, opts$W, 
                           L=opts$L, rho=opts$rho,
                           mu0=opts$mu0, 
                           B=Blag,
                           alpha=opts$alpha,
  ...
  if (model_compare[3]==1) {
    vb_out = vb_poisson(y,opts$model_code,
                        eta_select,
                        eta_init,
                        eta_prior_type,
                        eta_prior_val,
  ...
  if (model_compare[4]==1) {
    hvb_out = hva_poisson(y,opts$model_code,
                          eta_select[1:4], 
                          eta_init[1:4],
                          eta_prior_type[1:4], 
                          eta_prior_val[,1:4],
  ...
  if (model_compare[5]==1) {
    mcmc_out = mcmc_disturbance_pois(y,
                                     opts$model_code,
                                     eta_select,
                                     eta_init,
                                     eta_prior_type,
  ...
  p1 = plot_psi(psi_list=psi_list, ytrue=y, opts=opts,
                plot_hpsi=TRUE,plot_lambda=TRUE)
