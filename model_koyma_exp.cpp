...
<link> lambda[t] = phi[1] exp(psi[t]) y[t-1] + phi[2] exp(psi[t-1]) y[t-2] + ... + phi[L] exp(psi[t-L+1]) y[t-L]
<state> psi[t] = psi[t-1] + omega[t], omega[t] ~ N(0,W)


------ Dynamic Linear Model Form ------
<obs> y[t] | lambda[t] = Ft(theta[t]), 
    where Ft(theta[t]) = phi[1] y[t-1] exp(theta[t,1]) + ... + phi[L] y[t-L] exp(theta[t,L])
...
void forwardFilterW(
    ...
    */
    ...
    arma::vec Ft = arma::exp(at.col(1)) % Fphi % Fy; // Ft(theta[t]) evaluated at theta[t] = a[t]
...
//' @export
// [[Rcpp::export]]
Rcpp::List lbe_pois_koyama_exp_W(
...
//' @export
// [[Rcpp::export]]
Rcpp::List lbe_pois_koyama_exp_Wt(
	const arma::vec& Y, // n x 1, the observed response
    const unsigned int L = 12,
...
//' @export
// [[Rcpp::export]]
arma::mat mcf_pois_koyama_exp_W(
    const arma::vec& Y, // n x 1, the observed response
	const double W,
...
//' @export
// [[Rcpp::export]]
arma::mat mcs_pois_koyama_exp_W(
    const arma::vec& Y, // n x 1, the observed response
	const double W,
    const unsigned int L = 12, // number of lags
...
//     for (unsigned int r=1; r<n; r++) { // loop through row
//         tmpi = std::min(L-1,r-1);
//         Fy.zeros();
//         Fy.head(tmpi+1) = arma::reverse(Y.subvec(r-1-tmpi,r-1)); 
...
//     arma::vec Fphi = get_Fphi(L);
// 	arma::mat Fx = update_Fx_Koyama_max(n,L,Fphi,Y);
