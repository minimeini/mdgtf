...
Rcpp::List hva_poisson(
    ...
    const bool &truncated = true)
{
    Dim dim(Y.n_elem, nlag_in, lag_par_in.at(1));
    LagDist dlag(lag_dist, lag_par_in.at(0), lag_par_in.at(1));
    ObsDist dobs(obs_dist, obs_par_in.at(0), obs_par_in.at(1));
    ...
    */
    ...
    for (unsigned int s = 0; s < ntotal; s++)
    {
        ...
        */
        ...
        {
            theta_pad.at(t) = TransFunc::func_ft(
                t, ypad, theta_pad, psi_pad,
                dim, dlag, gain_func, trans_func);
        }
