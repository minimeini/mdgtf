get_posterior_summary = function(output, mod, param_infer) {
    ...
    for (par in param_infer) {
        if (par == "alpha") {
            par_qt = rbind(
                par_qt,
                t(apply(output$log_alpha, 1, quantile, qt))
            )
            par_name = c(
                par_name,
                paste("log_alpha", 1:nrow(output$log_alpha))
            )
            par_true = c(par_true, mod$param$spatial_effect)
        } else if (par == "car") {
            par_qt = rbind(
                par_qt,
                quantile(c(output$car_mu), qt),
                quantile(c(output$car_tau2), qt),
                quantile(c(output$car_rho), qt)
            )
            par_name = c(
                par_name,
                c("car_mu", "car_tau2", "car_rho")
            )
            par_true = c(par_true, mod$spatial$car)
        } else if (par == "par1") {
            par_qt = rbind(
                par_qt,
            ...
            par_qt = rbind(
                par_qt,
                quantile(c(output$beta), qt)
            )
            par_name = c(par_name, "beta")
            par_true = c(par_true, mod$param$beta)
        } else if (par == "rho") {
            par_qt = rbind(
                par_qt,
...
get_gelman_rubin = function(file_list, param_infer) {
    ...
    for (par in param_infer) {
        if (par == "alpha") {
            for (s in 1:nrow(output[[1]]$log_alpha)) {
                par_rhat = c(
                    par_rhat, 
                    gelman_rubin_cpp(
                        t(do.call(rbind, lapply(output, function(x) c(x$log_alpha[s,]))))
                    )
                )
                par_name = c(par_name, paste("log_alpha", s))
            }
        } else if (par == "car") {
            par_rhat = c(
                par_rhat,
                gelman_rubin_cpp(
                    t(do.call(rbind, lapply(output, function(x) c(x$car_mu))))
                ),
                gelman_rubin_cpp(
                    t(do.call(rbind, lapply(output, function(x) c(x$car_tau2))))
                ),
                gelman_rubin_cpp(
                    t(do.call(rbind, lapply(output, function(x) c(x$car_rho))))
                )
            )
            par_name = c(
                par_name,
            ...
            par_rhat = c(
                par_rhat, 
                gelman_rubin_cpp(
                    t(do.call(rbind, lapply(output, function(x) c(x$beta))))
                )
            )
            par_name = c(par_name, "beta")
        } else if (par == "rho") {
