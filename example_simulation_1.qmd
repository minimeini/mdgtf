...
Run `quarto render /Users/meinitang/Dropbox/Repository/poisson-dlm/example_simulation_1.qmd --to pdf` to compile.
...
```{r}
#| label: mis-specified model

mod1a <- mod1
mod1a$param$obs[2] <- 60 # less dispersion

mod1b <- mod1
mod1b$param$obs[2] <- 5 # more dispersion

mod1c <- mod1
mod1c$param$lag[1] <- 0.5 # smaller lag mean

mod1d <- mod1
mod1d$param$lag[1] <- 3 # larger lag mean

mod1e <- mod1
mod1e$param$lag[2] <- 1 # larger lag var

mod1f <- mod1
mod1f$param$lag[2] <- 0.05 # smaller lag var

mod1g <- mod1
mod1g$param$obs[1] <- 0.01 # smaller obs mean

mod1h <- mod1
mod1h$param$obs[1] <- 2 # larger obs mean
```
...
If inferring both $W$ and `par1`, the correlation of two trajectories is -0.87.
If inferring both $W$ and `par2`, the correlation of two trajectories is -0.5.
If inferring both `par1` and `par2`
...
Comparing three gain functions: ramp, softplus, and exponential
