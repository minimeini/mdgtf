...
param <- list(
    obs = c(1, 60),
    lag = c(0.4, 6), # (kappa, r): mode = 2.8969
    err = c(0.01, 0)
)
dim <- list(
    nlag = qnbinom(0.99, param$lag[2], 1 - param$lag[1]),
    ntime = 200,
    truncated = TRUE,
...
```{r}
#| label: run dgtf_infer with hva (mu0 and W)
#| include: false
#| eval: false
...
if (exists("out1.hva2")) {
    out1.hva2.loss_all = print_loss_all(out1.hva2)
    print(out1.hva2.loss_all)
}
```
